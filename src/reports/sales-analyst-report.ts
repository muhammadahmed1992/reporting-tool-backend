...
export class SalesAnalystReport implements ReportStrategy {
    ...
    public async generateReport(queryString: QueryStringDTO): Promise<ApiResponse<any>> {
        ...
        let countQuery = `
    SELECT COUNT(DISTINCT b.cstdcode) AS total_rows
    FROM (
        -- Subquery to handle invoice details and aggregation properly
    ...
    INNER JOIN (
        -- Main query with distinct stock codes
        SELECT DISTINCT civdfkinv, cstdcode
        FROM invoice
...
if (warehouse) {
    countQuery += ` AND (IFNULL(?, cinvfkwhs) = cinvfkwhs OR cinvfkwhs IS NULL)`;
    countParameters.push(warehouse);
}

if (stockGroup) {
    countQuery += ` AND (IFNULL(?, cstkfkgrp) = cstkfkgrp OR cstkfkgrp IS NULL)`;
    countParameters.push(stockGroup);
}

countQuery += ` ) AS b ON a.civdfkinv = b.civdfkinv;`;
...
group by cstdcode,cstkdesc,cexcdesc
 ) AS c, (SELECT @currentGroup := '', @currentSum := 0, @currentGroupAmountTax := '', @currentSumAmountTax := 0) r order by ${sortBy} ${sortOrder}
LIMIT ? OFFSET ?`;
