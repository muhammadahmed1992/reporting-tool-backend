...
where (IFNULL(?, cstdcode) = cstdcode or cstdcode is null)
group by cIvdFkStk,Location
        `;
        const [stockCode] = params;
        console.log(decodeURIComponent(stockCode));
        
        const response = await this.genericRepository.query<StocBalancekDTO>(query, [decodeURIComponent(stockCode)]);
        if (response?.length) {
